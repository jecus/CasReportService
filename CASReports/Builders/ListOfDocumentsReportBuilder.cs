...
{
	public class ListOfDocumentsReportBuilder : AbstractReportBuilder
	{
		#region Fields

		private Operator _reportedOperator;

		private string _reportTitle = "LIST OF DOCUMENTS";
		...
		public ListOfDocumentsReportBuilder(Operator @operator, IEnumerable<Document> documents)
		{
		...
		{
			var report = new ListOfDocumentsReport();
		...
		private void AddDocumentsToDataSet(DocumentDataSet destinationDateSet)
		{
			foreach (var reportedDocument in _reportedDocuments)
			{
				...
				destinationDateSet.DocumentDataTable.AddDocumentDataTableRow(title, contractNumber, description,
					remarks, nomenclature, issueValidFrom, docType,
					null, null, null, null, null, null,
					null, null, null, null, null, specialist);
			}
		}
