...
{
	public class RegisterOfDocumentReportBuilder : AbstractReportBuilder
	{
		#region Fields

		private Operator _reportedOperator;

		private string _reportTitle = "REGISTER OF DOCUMENTS";
		...
		public RegisterOfDocumentReportBuilder(Operator @operator, IEnumerable<Document> documents)
		{
		...
		{
			var report = new RegisterOfDocumentsReport();
		...
		private void AddDocumentsToDataSet(DocumentDataSet destinationDateSet)
		{
			foreach (var reportedDocument in _reportedDocuments)
			{
				...
				destinationDateSet.DocumentDataTable.AddDocumentDataTableRow(title, contractNumber, description,
					...
					null, null, null, null, null, null, null, specialist);
			}
		}
